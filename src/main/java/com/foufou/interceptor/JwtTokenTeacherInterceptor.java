...
public class JwtTokenTeacherInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        try {
            ...
            Claims claims = JwtUtil.parseJWT(jwtProperties.getTeaSecretKey(), token);
            Long userId = Long.valueOf(claims.get(JwtClaimsConstant.TEACHER_ID).toString());
            ThreadLocalUtil.set(userId);
            log.info("已存放id:{}", (Long) ThreadLocalUtil.get());
            //3、通过，放行
            return true;
        } catch (Exception ex) {
            //4、不通过，响应401状态码
